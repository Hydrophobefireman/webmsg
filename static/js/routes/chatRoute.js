...
function chatComponentOnUnmount() {
  ...
  for (const a of Object.keys(_GlobalScope._functions))
    _GlobalScope._functions[a] = noop;
  Object.keys(_GlobalScope).forEach(a => {
    delete _GlobalScope[a];
  }),
    (_GlobalScope.websockets = []),
    (_GlobalScope._functions = {}),
    (_GlobalScope.chatWith = null);
...
const startWebSocketConn = async a => {
  ...
  return (
    c &&
      ...
      (_GlobalScope.websockets.length = 0)),
    _GlobalScope.websockets.push(b),
    b
  );
};
...
const pcOnicecandidate = async ({ candidate }, c) => {
  if (candidate) {
    await sendMessage({
      sendTo: c,
      rtcData: {
...
function __startSessionPing(dc) {
  ...
  if (_msg === "__ping__") {
    document.title = `WebMsg ${
      _GlobalScope.chatWith ? `Chat With - ${_GlobalScope.chatWith}` : ""
    }`;
...
async function parseRTCData(data, c) {
  const { rtcData, sendTo: chat } = data;
  if (c !== chat) {
...
const chatComponentOnRender = async (args, app) => {
  ...
  if (!validChat) {
    return app.pushStatus(
      500,
      "the chat id you provided is invalid. please try again"
    );
  }
