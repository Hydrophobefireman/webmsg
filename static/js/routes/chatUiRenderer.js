...
const _startFilePicker = (e, dc) => {
    ...
    reader.onload = () => sendRTCMessage(dc, reader.result, !0, {
        name,
        type,
        size,
    });
...
async function createUIChanges(opts) {
    ...
    createChatBox(chat, app, inponkd(dc), startAddAttachment(dc), logoutFn(pc, dc));
};
...
function _renderSingleMessage(_msg, saveToDb) {
    ...
    if (data && !binary) {
        ...
        const _node = document.createTextNode(`File:${name} Size:${apptSize(size)} type:${type}`);
        const i = new Image;
        el.appendChild(i);
...
function messageonClickFn(el) {
    ...
    if (media) {
        ...
        media_val.onclick = () => {
            ...
            const _src = URL.createObjectURL(arrayBufferToBlob(el._messagedata, val.type));
            img.src = _src
            $par_const.innerHTML = '';
            const i = $.create("a");
...
async function getChatMessages(a, b) {
    ...
    return c || (
            b && notifyUser("No IndexedDB Support Found", {
...
function _receiveMessage(e) {
    ...
    function _binaryMessageHandler(_) {
        ...
        if (_BCount === finalBuf.byteLength) {
            trace("Data Received!");
            _renderSingleMessage({ ...binarymeta,
                data: finalBuf.buffer,
                binary: true
            }, true);
...
async function sendBinaryData(dc, data, _m) {
    ...
    dc.onmessage = () => null
    const DCMsg = () => new Promise(resolve => {
...
function _startedTyping(a, b = $.id("__chat-with-prof")) {
    clearTimeout(_t_out_typing),
        a === CACHE.USERS.THERE && b && (b.textContent = `${a} is typing...`,
