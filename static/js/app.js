((async () => {
    ...
    async function _paintChatPage(params) {
        $.empty(root);
        root.innerHTML = "Loading Chat Data"
        const id = params[0];
        ...
        async function createChatBox(u) {
            ...
            close_btn.onclick = () => {
                if (typeof ws === "object") {
                    ws.onclose = () => {}
                    app.__websockets__ = app.__websockets__.filter(x => x.close() || x);
                    //close any and all websockets
                };
                app.route(`/u/${HERE}`);
            };
            $menu.id = "Nzk3NzEzOD"
        ...
        async function checkForMessages() {
            ...
            let _timeout;
            _timeout = _CustomSetTimeout(await checkForMessages, 5 * 1000);
        }
    ...
    function fetchData(obj) {
        return new Promise((res, rej) => {
            ...
            ws.onopen = () => {
                res(ws.send(js));
                ws.close();
                app.__websockets__ = app.__websockets__.filter(x => x !== ws);
            }
        });
    }
}))()